...
module.exports = router => {
    ...
    router.post('/registrasi', (req, res) => {
        ...
        if (!username || !namalengkap || !email || !nik || !alamat || !nohp || !password || !username.trim() || !namalengkap.trim()
            || !email.trim() || !nik.trim() || !alamat.trim() || !nohp.trim() || !password.trim()) {

            res.status(400).json({message: 'Gagal'});

        } else {

            register.registerUser(username, namalengkap, email, nik, alamat, nohp, password)
